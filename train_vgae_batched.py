...
from networks.vgae_model import MyModel    
    ...
    for i in range(EPOCHS):
        ...
        for i in range(0, min(len(train_edges), train_false_edges.shape[0]), batch_size):
            ...
            with tf.GradientTape() as tape:    
                # forward pass
                embs, mu, logvar = model(feature_tensor, training=True)
        ...
        embs, mu, logvar = model(feature_tensor, training=False)
...
def test(features, model, test_p, test_n, dataset, clust):
    ...
    embs, _, _ = model(feature_tensor, training=False)
...
def complete_graph(node_to_clust):
    clust = "complete"
    adj_train, features, test_matrix, valid_matrix  = get_complete_data(DATASET_NAME, N_CLUSTERS)
