...
def train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, intra_train_edges, intra_valid_edges, intra_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust=None):
    ...
    if condition:
        print("MATRIX TRAIN")
        return matrix_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, intra_train_edges, intra_valid_edges, intra_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust)
    else:
        print("BATCHED TRAIN")
        return batched_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, intra_train_edges, intra_valid_edges, intra_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust)


def matrix_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, intra_train_edges, intra_valid_edges, intra_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust=None):
    train_accs = []
    train_losses = []
    ...
    # define the true value for the association node-cluster
    cluster_labels = []
    for i in range(n_clusters):
        if LABEL_OF_ALL_1:
            ith_label = [1/n_clusters]*n_clusters
        else:
            ith_label = [0]*n_clusters
            ith_label[i] = 1
        cluster_labels += [ith_label] * n_nodes[i]
    ...
    model = LastSharedWithAdversarialLoss(adj_train_norm_tensor)
    ...
    for epoch in range(EPOCHS):
        ...
        with tf.GradientTape() as tape:  
            ...
            if (epoch % NUM_EPOCHS_ADV_LOSS != 0):
                ...
                predicted_classes = []
                for clust in range(n_clusters):
                    predicted_classes.append(model(feature_tensor[clust], cluster=clust, training=True, predict_cluster=True))

                [print(pred.shape) for pred in predicted_classes]
                ...
                optimizer.apply_gradients(zip (grad, model.conv_1.trainable_variables))
...
def batched_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, intra_train_edges, intra_valid_edges, intra_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust=None):
    train_accs = []
    train_losses = []
    ...
    for epoch in range(EPOCHS):
        ...
        if(epoch%NUM_EPOCHS_ADV_LOSS !=0):
            ...
            with tf.GradientTape() as tape:  
                ...
                predicted_clusters = tf.concat(predicted_classes, 0)
                print(f"single_pred shape: {predicted_clusters[0].shape}")
...
if __name__ == "__main__":
    ...
    model = train(features_, adj_train_list, adj_train_norm_list, train_edges_list, valid_edges_list, valid_false_edges_list, 
                    intra_train_edges, intra_valid_edges, intra_valid_false_edges, adj_train, clust_to_node, com_idx_to_clust_idx, node_to_clust=node_to_clust)
    ...
    if(LEAVE_INTRA_CLUSTERS):
        ...
        test_ap, test_auc, cms_between = test_in_between_edges(features_, model, pos_edges, neg_edges, DATASET_NAME, "share_last_with_adversarial_loss")
