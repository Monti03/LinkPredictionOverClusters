...
from batched_train import train as complete_train
from batched_train import test as complete_test
...
def train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, share_first, node_to_clust=None):
    train_accs = []
    train_losses = []
        ...
        for i in range(max_batches):
            ...
            with tf.GradientTape() as tape:  
                ...
                for clust in range(n_clusters):
                    ...
                    # sa above, but for the false edges
                    tmp_train_edges = clust_train_false_edges[i * batch_sizes[clust]: (i+1) * batch_sizes[clust]]
                    tmp_from = tmp_train_edges[:,0]
                    tmp_to = tmp_train_edges[:,1]
...
def get_preds(embs, edges_pos, edges_neg):
...
def test(features_list, model, test_p, test_n, dataset, clust):
    ...
    auc, ap = get_scores(labels_all, preds_all, dataset, clust)
...
if __name__ == "__main__":
    ...
    test_ap, test_auc = test(features_, model, test_edges_list, test_false_edges_list, DATASET_NAME, f"share_{model_name}")
