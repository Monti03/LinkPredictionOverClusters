...
def train_preds(embs_from, embs_to, tmp_train_edges, between_cluster_fc=None, cluster_1=None, cluster_2=None):
    ...
    if(USE_FCS):
        ...
        train_pos_pred = between_cluster_fc(embs_from, embs_to,tmp_train_edges, cluster_1, cluster_2)
    else:
...
def train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, between_train_edges, between_valid_edges, between_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust=None):
    ...
    if condition:
        print("MATRIX TRAIN")
        return matrix_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, between_train_edges, between_valid_edges, between_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust)
    else:
        print("BATCHED TRAIN")
        return batched_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, between_train_edges, between_valid_edges, between_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust)


def matrix_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, between_train_edges, between_valid_edges, between_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust=None):
    train_accs = []
    train_losses = []
    ...
    for epoch in range(EPOCHS):
        ...
        with tf.GradientTape() as tape:  
            ...
            for clust in range(n_clusters):
                ...
                train_pos_pred = tf.gather_nd(complete_graph_preds, clust_train_edges[:, :2])
                train_neg_pred = tf.gather_nd(complete_graph_preds, clust_train_false_edges[:, :2])
            ...
            for cluster_1 in range(n_clusters):
                for cluster_2 in range(cluster_1+1, n_clusters):
                    ...
                    tmp_train_edges_1 = between_train_edges[:,2]==cluster_1
                    tmp_train_edges_2 = between_train_edges[:,3]==cluster_2
                    tmp_train_edges_1_2 = tmp_train_edges_1 * tmp_train_edges_2

                    # get the right edges from clust1 to clust2
                    tmp_train_edges_1_2 = between_train_edges[tmp_train_edges_1_2]
                    ...
                    if(tmp_train_edges_1_2.shape[0] > 0):
                        ...
                        # obtain the logits by a scalar product of the embs of the two nodes of the corresponding edge
                        if(USE_FCS):
                            train_pos_pred = between_cluster_fc(embs_list[cluster_1], embs_list[cluster_2], tmp_train_edges_1_2, cluster_1, cluster_2, matrix_opetaion=True)
                        else:
                            train_pos_pred = tf.gather_nd(tf.matmul(embs_list[cluster_1], embs_list[cluster_2], transpose_b=True), tmp_train_edges_1_2[:,:2])

                    tmp_train_edges_1 = between_train_false_edges[:,2]==cluster_1
                    tmp_train_edges_2 = between_train_false_edges[:,3]==cluster_2
                    tmp_train_edges_1_2 = tmp_train_edges_1 * tmp_train_edges_2

                    tmp_train_edges_1_2 = between_train_false_edges[tmp_train_edges_1_2]
                    ...
                    if(tmp_train_edges_1_2.shape[0]>0):
                        ...
                        # obtain the logits by a scalar product of the embs of the two nodes of the corresponding edge
                        if(USE_FCS):
                            train_neg_pred = between_cluster_fc(embs_list[cluster_1], embs_list[cluster_2], tmp_train_edges_1_2, cluster_1, cluster_2, matrix_opetaion=True)                        
                        else:
                            train_neg_pred = tf.gather_nd(tf.matmul(embs_list[cluster_1], embs_list[cluster_2], transpose_b=True), tmp_train_edges_1_2[:,:2])
        ...
        for cluster_1 in range(n_clusters):
            for cluster_2 in range(cluster_1+1, n_clusters):
                ...
                if(tmp_valid_edges_1_2.shape[0] > 0):
                    # obtain the logits by a scalar product of the embs of the two nodes of the corresponding edge
                    if(USE_FCS):
                        valid_pos_pred = between_cluster_fc(embs_list[cluster_1], embs_list[cluster_2], tmp_valid_edges_1_2, cluster_1, cluster_2, matrix_opetaion=True)
                    else:
                        valid_pos_pred = tf.gather_nd(tf.matmul(embs_list[cluster_1], embs_list[cluster_2], transpose_b=True), tmp_valid_edges_1_2[:,:2])
                ...
                if(tmp_valid_edges_1_2.shape[0] > 0):
                    # obtain the logits by a scalar product of the embs of the two nodes of the corresponding edge
                    if(USE_FCS):
                        valid_neg_pred = between_cluster_fc(embs_list[cluster_1], embs_list[cluster_2], tmp_valid_edges_1_2, cluster_1, cluster_2, matrix_opetaion=True)
                    else:
                        valid_neg_pred = tf.gather_nd(tf.matmul(embs_list[cluster_1], embs_list[cluster_2], transpose_b=True), tmp_valid_edges_1_2[:,:2])
...
def batched_train(features_list, adj_train_list, adj_train_norm_list, train_edges, valid_edges, valid_false_edges, between_train_edges, between_valid_edges, between_valid_false_edges, complete_adj, clust_to_node, com_idx_to_clust_idx, node_to_clust=None):
    train_accs = []
    train_losses = []
                ...
                for cluster_1 in range(n_clusters):
                    for cluster_2 in range(cluster_1+1, n_clusters):
                        ...
                        if(tmp_train_edges_1_2.shape[0] > 0):
                            # obtain the logits by a scalar product of the embs of the two nodes of the corresponding edge
                            if(USE_FCS):
                                train_pos_pred = between_cluster_fc(embs_list[cluster_1], embs_list[cluster_2], tmp_train_edges_1_2, cluster_1, cluster_2)
                            else:
                                # get the train edges of this batch
                        ...
                        if(tmp_train_edges_1_2.shape[0]>0):
                            # obtain the logits by a scalar product of the embs of the two nodes of the corresponding edge
                            if(USE_FCS):
                                train_neg_pred = between_cluster_fc(embs_list[cluster_1], embs_list[cluster_2], tmp_train_edges_1_2, cluster_1, cluster_2)
                            else:
                                # get the train edges of this batch
                                tmp_from = tmp_train_edges_1_2[:,0]
                                tmp_to = tmp_train_edges_1_2[:,1]
    ...
    for i in range(0, len(edges_pos), BATCH_SIZE):
        ...
        if use_fcs_condition:
            ...
            if cluster_2 > cluster_1:
                batch_logits= between_cluster_fc(embs, embs_1, tmp_valid_edges, cluster_1, cluster_2)
            else:
                tmp_valid_edges = np.flip(tmp_valid_edges[:,:2], axis=1) # exchange the first with the second column
                batch_logits= between_cluster_fc(embs_1, embs, tmp_valid_edges, cluster_2, cluster_1)
    ...
    for i in range(0, len(edges_neg), BATCH_SIZE):
        ...
        if use_fcs_condition:
            ...
            if cluster_2 > cluster_1:
                ...
                tmp_valid_edges = np.flip(tmp_valid_edges, axis=1)
                batch_logits= between_cluster_fc(embs_1, embs, tmp_valid_edges, cluster_2, cluster_1)
...
if __name__ == "__main__":
    ...
    if(LEAVE_INTRA_CLUSTERS):
        ...
        if USE_FCS:
            ...
            test_ap, test_auc, cms_between = test_in_between_edges(features_, model, pos_edges, neg_edges, DATASET_NAME, model_name)
